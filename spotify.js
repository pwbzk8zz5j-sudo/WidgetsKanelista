const WORKER_URL = "https://aged-cell-ee03.rigivaldi.workers.dev";
...
async function fetchPlayback({ silent = false } = {}) {
  ...
  try {
    ...
    if (!data.connected) {
      throw new Error("Spotify todavía no está conectado al Worker.");
    }

    if (!data.item) {
      renderIdle("No hay una canción activa en Spotify.");
      return;
    }
